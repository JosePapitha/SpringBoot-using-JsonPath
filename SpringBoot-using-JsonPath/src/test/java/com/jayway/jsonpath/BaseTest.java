...
public class BaseTest {
	 ...
	 * .builder() .mappingProvider(new JsonOrgMappingProvider()) .jsonProvider(new
	 * JsonOrgJsonProvider()) .build();
	 *
	 * public static final Configuration GSON_CONFIGURATION = Configuration
	 * .builder() .mappingProvider(new GsonMappingProvider()) .jsonProvider(new
	 * GsonJsonProvider()) .build();
	 *
	 * public static final Configuration JACKSON_CONFIGURATION = Configuration
	 * .builder() .mappingProvider(new JacksonMappingProvider()) .jsonProvider(new
	 * JacksonJsonProvider()) .build();
	 *
	 ...
	 * .builder() .jsonProvider(new JettisonProvider()) .build();
	 */

	/*
	 * public static final Configuration JSON_SMART_CONFIGURATION = Configuration
	 * .builder() .mappingProvider(new JsonSmartMappingProvider()) .jsonProvider(new
	 * JsonSmartJsonProvider()) .build();
	 */

	/*
	 * public static final Configuration TAPESTRY_JSON_CONFIGURATION = Configuration
	 * .builder() .mappingProvider(new TapestryMappingProvider())
	 * .jsonProvider(TapestryJsonProvider.INSTANCE) .build();
	 *
	 * public static final Configuration JAKARTA_JSON_CONFIGURATION = Configuration
	 * .builder() .mappingProvider(new JakartaMappingProvider()) .jsonProvider(new
	 * JakartaJsonProvider()) .build();
	 *
	 * // extension to Jakarta EE 9 JSON-P with mutable objects and array public
	 * static final Configuration JAKARTA_JSON_RW_CONFIGURATION = Configuration
	 * .builder() .mappingProvider(new JakartaMappingProvider()) .jsonProvider(new
	 * JakartaJsonProvider(true)) .build();
	 */
	 ...
	 * Configuration.defaultConfiguration(), new HashMap<Path, Object>()); }
	 */
}
