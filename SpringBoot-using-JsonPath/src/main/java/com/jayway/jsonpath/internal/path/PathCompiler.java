...
public class PathCompiler {
    ...
    public static Path compile(String path, final Predicate... filters) {
        try {
            ...
            LinkedList<Predicate> filterStack = new LinkedList<>(asList(filters));
    ...
    private boolean readFilterToken(PathTokenAppender appender) {
        ...
        if ((closeBracketIndex == -1) || !path.nextSignificantCharIs(closeBracketIndex, CLOSE_SQUARE_BRACKET)) {
            return false;
        }
    ...
    private boolean readWildCardToken(PathTokenAppender appender) {
        ...
        if ((inBracket && !path.nextSignificantCharIs(WILDCARD)) || (!path.currentCharIs(WILDCARD) && path.isOutOfBounds(path.position() + 1))) {
            return false;
        }
        if (inBracket) {
